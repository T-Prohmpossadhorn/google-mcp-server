...
// NewMultiAccountClient creates a new multi-account Sheets client and eagerly
...
func NewMultiAccountClient(ctx context.Context, accountManager *auth.AccountManager) (*MultiAccountClient, error) {
	...
	for email, oauthClient := range accountManager.GetAllOAuthClients() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "Warning: failed to create sheets service for %s: %v\n", email, err)
			continue
		}
...
// GetClientForContext returns the Sheets client for the account resolved from
// the provided hint (an email address), creating one on demand if needed.
...
// NewMultiAccountHandler creates a new Sheets handler with multi-account support.
...
func NewMultiAccountHandler(accountManager *auth.AccountManager, defaultClient *Client) *MultiAccountHandler {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "Warning: failed to initialize sheets multi-account client: %v\n", err)
		multiClient = &MultiAccountClient{
			accountManager: accountManager,
			clients:        make(map[string]*Client),
		}
...
// GetTools returns the Sheets tools, each augmented with an optional account
...
// HandleToolCall routes a Sheets tool call to the client for the requested account.
...
// HandleResourceCall handles a resource call for the Sheets service.
